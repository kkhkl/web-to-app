...
WebToApp 宿主本身有意保持 `targetSdk = 28`，以便像 Termux 那样从 app 存储里 fork、exec 本地原生二进制，所以宿主通过 GitHub Releases 分发。

但生成的应用完全可以上架 Google Play：设备端导出 AAB 时会自动把 `targetSdk` 改写到 Play 当前要求（目前为 35）并在本地签名，因此 Web、HTML、前端、媒体类应用都能直接上架 Play 商店。依赖 fork 本地运行时的应用（Node.js、PHP、Python、Go、WordPress）与 Play 不兼容，只能走 APK 分发。
...
- 宿主有意保持 `targetSdk = 28`，以便本地原生运行时从 app 存储中 `fork` 和 `exec`；导出 AAB 时会单独把 `targetSdk` 改写以满足 Google Play 上架要求。
